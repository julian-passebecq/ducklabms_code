...
export type Resource = NotebookResource | CatalogResource | EvidenceResource | GraphResource;
export interface Point {x:number; y:number}
export interface ViewInstance {id:string; resource_id:string; mode:'visual'|'spec'; positions:Record<string,Point>; viewport:{x:number;y:number;zoom:number}|null; selected_node:string|null; selected_edge:string|null}
export interface Pane {id:string; view_ids:string[]; active_view_id:string|null; weight:number}
export interface RootWorkbench {schema_version:1; resources:Resource[]; views:ViewInstance[]; panes:Pane[]; active_pane_id:string; persona:Persona; direction:'horizontal'|'vertical'}

/** Display-only projection, not a universal domain graph or execution plan. */
export interface GraphProjection {nodes:Array<{id:string;label:string;detail:string}>; edges:Array<{id:string;source:string;target:string;label:string}>}
...
export const legacyTruth:Record<TruthKind,EvidenceKind> = {real:'real_local','semantic-emulation':'emulated',simulated:'simulated',unsupported:'unavailable'};
export function evidenceOf(run:Execution):{semantic:EvidenceKind;simulation:EvidenceKind;measured_local_ms:number|null;modeled_duration_s:number|null} {
 return {semantic:run.status!=='success'?'unavailable':['sparklab','dbt'].includes(run.language)?'emulated':'real_local',
  simulation:run.simulation?.status==='modeled'?'simulated':'unavailable',measured_local_ms:run.elapsed_ms??null,
