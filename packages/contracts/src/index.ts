...
export interface RuntimeClient {execute(workspaceId:string,request:ExecuteRequest):Promise<Execution>;exercise(workspaceId:string,request:ExerciseRequest):Promise<ExerciseResult>;catalog(workspaceId:string):Promise<Asset[]>;lakehouse(workspaceId:string):Promise<LakehouseOverview>;airflowCapabilities():Promise<AirflowRemoteCapabilities>;airflowDispatch(request:AirflowDispatchRequest):Promise<AirflowDispatch>;airflowStatus(runId:number):Promise<AirflowRunStatus>;airflowResult(runId:number,requestId:string):Promise<AirflowRunResult>;restart(workspaceId:string):Promise<unknown>}
