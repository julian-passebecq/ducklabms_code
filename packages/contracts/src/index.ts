...
export interface SparkRemoteCapabilities {enabled:boolean;mode:'github_actions_ephemeral';spark_version?:string;master?:string;repo?:string;workflow?:string;ref?:string;public_repo?:boolean;truth:string;cluster_truth:string;privacy?:string;proxy_truth?:string;limits?:{code_bytes:number;tables_json_bytes:number;job_timeout_minutes:number;execution_timeout_seconds:number}}
export interface SparkRemoteDispatchRequest {notebook_id:string;code:string;collect_limit:number}
export interface SparkRemoteDispatch {request_id:string;status:'accepted';run_id:number;run_url:string|null;truth:string;fixture_truth?:string;sources?:string[]}
export interface SparkRemoteRunStatus {status:string;conclusion:string|null;run_id:number;run_url:string|null;created_at?:string;run_started_at?:string;updated_at?:string;artifact_available:boolean;truth:string}
