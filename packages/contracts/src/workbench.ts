...
import {isGraph,projectGraph,type GraphResource,type Resource,type RootWorkbench,type ViewInstance} from './foundation.ts';
...
export function replaceResource(state:RootWorkbench,resource:Resource):RootWorkbench {
 ...
 if(isGraph(resource)){const graph=projectGraph(resource),nodes=new Set(graph.nodes.map(n=>n.id)),edges=new Set(graph.edges.map(e=>e.id));for(const v of next.views.filter(v=>v.resource_id===resource.id)){v.positions=Object.fromEntries(Object.entries(v.positions).filter(([id])=>nodes.has(id)));if(v.selected_node&&!nodes.has(v.selected_node))v.selected_node=null;if(v.selected_edge&&!edges.has(v.selected_edge))v.selected_edge=null}}
...
export function removeResource(state:RootWorkbench,id:string):RootWorkbench {
 if(state.resources.some(r=>r.kind==='workflow'&&r.tasks.some(t=>t.notebook_resource_id===id)))throw new Error('Unlink workflow task references before removing this notebook reference.');
 let next=state;for(const view of state.views.filter(v=>v.resource_id===id))next=closeView(next,view.id);
 next={...next,resources:next.resources.filter(r=>r.id!==id)};assertWorkbench(next);return next;
}
