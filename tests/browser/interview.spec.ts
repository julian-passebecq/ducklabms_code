...
test('standalone practice shares notebook, saves drafts and separates Run from Submit',async({page,request})=>{
 ...
 await expect(page.getByRole('tab',{name:'Interview',exact:true})).toHaveAttribute('aria-selected','true');
 ...
 await page.getByRole('button',{name:'Submit',exact:true}).click();await idle();
 ...
 await expect(page.getByText('SELECT COALESCE(SUM(value), 0) AS total FROM input',{exact:true}).first()).toBeVisible();
 ...
 await page.screenshot({path:'evidence/interview-pass-1-narrow.png',fullPage:true});
