...
export interface ToolContext {workspaceId:string;notebookId:string;services:{runtime:RuntimeClient;inspectAsset:(name:string)=>void;compactAsset:(name:string)=>void};caseStudy:CaseStudy;assets:Asset[];runs:Execution[];selectedStep:string;onSelectStep:(id:string)=>void;onRunWorkflow:()=>void;busy:boolean}
...
export function CatalogSurface({context,onInspect}:{context:ToolContext;onInspect:(name:string)=>void}){
 const candidates=context.assets.filter(a=>a.storage?.health==='small_files'&&a.layer!=='source');
 return <div className="surface-page"><div className="section-eyebrow">SHARED WORKSPACE CATALOG</div><h1>The same data in every tool</h1><p className="lead">Publish a table in one notebook and query it from another module. A new upstream version makes dependent assets stale. DuckLake tables expose measured Parquet layout and snapshot evidence.</p><div className="asset-grid">{context.assets.map(a=><button className="asset-card" key={a.name} onClick={()=>onInspect(a.name)}><div><Badge appearance="tint" color={a.fresh?'success':'warning'}>{a.fresh?'fresh':'stale'}</Badge><span>{a.layer}</span></div><h3>{a.name}</h3><b>{a.row_count.toLocaleString()} rows</b>{a.storage&&<><small>{a.storage.file_count} Parquet file{a.storage.file_count===1?'':'s'} · {formatStorageBytes(a.storage.size_bytes)}{a.storage.snapshot_id!==null?' · snapshot '+a.storage.snapshot_id:''}</small><small>{a.storage.health==='small_files'?'Small-file maintenance candidate':'No small-file signal'}{a.storage.avg_file_size_bytes!==undefined?' · avg '+formatStorageBytes(a.storage.avg_file_size_bytes):''}{a.storage.delete_file_count?' · '+a.storage.delete_file_count+' delete file(s)':''}</small><small>Pruning: zone-map opportunity only; actual scanned/pruned files are not measured.</small></>}<p>{a.producer??'No producer recorded'}</p><small>{Object.keys(a.inputs??{}).join(' + ')||'Independent source'}</small></button>)}</div>{candidates.length>0&&<section><h2>DuckLake maintenance lab</h2><p className="notice">These actions call DuckLake's real adjacent-file compaction for the selected derived table. Row semantics and Datapass logical asset versions must remain unchanged. A lower file count is measured; a faster query is not assumed.</p><div className="tag-row">{candidates.map(a=><Button key={a.name} size="small" disabled={context.busy} onClick={()=>context.services.compactAsset(a.name)}>Compact {a.name} ({a.storage?.file_count} files)</Button>)}</div></section>}</div>;
}
