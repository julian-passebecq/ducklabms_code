...
export function buildRegistry(manifests:ModuleManifest[]):Map<string,ToolPlugin>{
 return new Map(manifests.map(manifest=>{
  ...
  const mapped=(modulePanels[manifest.id]??['brief']).map(id=>{
   if(manifest.id==='sparklab'&&id==='graph')return {id,render:(context:ToolContext)=><LocalSparkTeachingSurface context={context}/>} as ToolPanel;
   if(manifest.id==='airflow'&&id==='graph')return {id,render:(context:ToolContext)=><PipelineSurface context={context}/>} as ToolPanel;
   return panels[id];
  });
