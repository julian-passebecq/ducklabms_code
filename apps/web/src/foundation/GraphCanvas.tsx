...
function GraphNode({data,selected}:NodeProps){return <div className={`foundation-graph-node ${selected?'is-selected':''}`}><Handle type="target" position={Position.Left}/><Badge size="small" appearance="outline">Design</Badge><strong>{String(data.label)}</strong><small>{String(data.detail)}</small><Handle type="source" position={Position.Right}/></div>}
const nodeTypes={foundation:GraphNode};
interface Props {graph:GraphProjection;view:ViewInstance;disabled:boolean;onView:(patch:Partial<ViewInstance>)=>void;onConnect:(source:string,target:string)=>void}
function Inner({graph,view,disabled,onView,onConnect}:Props){
 ...
 return <div className="foundation-graph-host" data-testid="foundation-graph">
  <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={changes}
   ...
   onConnect={c=>{if(!disabled&&c.source&&c.target)onConnect(c.source,c.target)}}
