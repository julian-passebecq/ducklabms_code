...
import type {WorkspacePresentation} from './workspacePresentation.ts';
import {normalizePreferredView} from './studio/desktopModel.ts';
import {preserveAnalyticsAttachment} from './studio/analytics/project.ts';

export type NotebookPlayground='ducklake'|'fabric'|'free'|'motherduck';
...
export interface RootNotebook {schemaVersion:1;id:string;title:string;blocks:RootBlock[];views:NotebookView[];blockState:Record<string,unknown>;info:ImportedNotebookInfo|null;executions:Record<string,Execution>;executedSource:Record<string,string>;practiceLayouts?:Record<string,NotebookView['layout']>;exercise?:{id:string;version:string};playground?:NotebookPlayground;revealedHints?:number;solutionRevealed?:boolean;skin?:'neutral'|'fabric'|'databricks';presentation?:WorkspacePresentation;preferredView?:string;outputCheckpoints?:Record<string,string>;clearedOutputs?:string[]}
...
export function restoreNotebook(value:unknown):RootNotebook {
 ...
 return {schemaVersion:1,id:typeof doc.id==='string'&&/^[A-Za-z0-9_-]{1,100}$/.test(doc.id)?doc.id:'case-notebook',title:String(doc.title??'Notebook'),blocks,views:normalized.views.filter(v=>!v.id.startsWith('saved-practice-')),practiceLayouts:Object.fromEntries(normalized.views.filter(v=>v.id.startsWith('saved-practice-')).map(v=>[v.id.slice(15),v.layout])),blockState:{...normalized.blockState,...preserveAnalyticsAttachment(doc.blockState)},info:normalized.notebookInfo,executions:{},executedSource:{},exercise:doc.exercise&&typeof doc.exercise.id==='string'&&typeof doc.exercise.version==='string'?doc.exercise:undefined,playground:['ducklake','fabric','free','motherduck'].includes(doc.playground??'')?doc.playground:undefined,solutionRevealed:doc.solutionRevealed===true,revealedHints:Number.isInteger(doc.revealedHints)?Math.max(0,Math.min(100,doc.revealedHints!)):0,skin:['neutral','fabric','databricks'].includes(doc.skin??'')?doc.skin:'neutral',presentation:['studio','fabric','leetcode'].includes(doc.presentation??'')?doc.presentation:(doc.exercise?'leetcode':'studio'),...(normalizePreferredView(doc.preferredView,normalized.views)?{preferredView:normalizePreferredView(doc.preferredView,normalized.views)}:{}),outputCheckpoints:Object.fromEntries(Object.entries(doc.outputCheckpoints??{}).filter(([id,run])=>byId.has(id)&&typeof run==='string')),clearedOutputs:Array.isArray(doc.clearedOutputs)?doc.clearedOutputs.filter(id=>typeof id==='string'):[]};
}
